        ...
        <>
            ...
            <div className="fixed top-0 left-0 right-0 z-50 flex justify-center py-4 px-4 pointer-events-none">
                <motion.nav
                    ...
                    className={`pointer-events-auto w-full max-w-6xl rounded-full border transition-all duration-300 ${scrolled
                        ? "bg-white/10 backdrop-blur-2xl border-white/20 shadow-2xl shadow-black/10"
                        : "bg-white/5 backdrop-blur-xl border-white/10 shadow-xl shadow-black/5"
                        } flex items-center justify-between pl-3 pr-3 py-3 md:pl-8 md:pr-3`}
                >
                    {/* Desktop Navigation Container */}
                    <div className="hidden md:flex items-center w-full justify-between">

                        {/* Left Side Links */}
                        <div className="flex items-center gap-1 bg-white/10 backdrop-blur-md p-2 rounded-full border border-white/20">
                                ...
                                return (
                                    ...
                                    >
                                        <Link
                                            ...
                                            className={`relative z-10 text-sm font-semibold transition-colors duration-200 flex items-center gap-1.5 ${hoveredItem === key ? "text-slate-900" : "text-slate-700 hover:text-slate-900"}`}
                                        >
                                            {section.title}
                                        ...
                                        {hoveredItem === key && (
                                            <motion.div
                                                ...
                                                className="absolute inset-0 bg-white/40 backdrop-blur-sm rounded-full shadow-lg border border-white/30"
                        ...
                        <Link href="/" className="flex items-center justify-center group mx-6">
                            <div className="relative w-12 h-12 overflow-hidden rounded-2xl bg-linear-to-br from-teal-500 to-cyan-500 shadow-xl shadow-teal-500/30 border border-white/20 flex items-center justify-center group-hover:scale-110 group-hover:shadow-2xl group-hover:shadow-teal-500/40 transition-all duration-300 z-20">
                                <Image
                        ...
                        <div className="flex items-center gap-1 bg-white/10 backdrop-blur-md p-2 rounded-full border border-white/20">
                            ...
                            <Link href="/about" className="relative px-5 py-2.5 text-sm font-semibold text-slate-700 hover:text-slate-900 transition-colors z-10">
                                About Us
                            </Link>

                            {/* Desktop Download Button inside Right Group */}
                            <div className="pl-2">
                                <AnimatedDownloadButton className="h-11 px-6 rounded-full text-sm font-bold shadow-lg hover:shadow-xl transition-all flex items-center justify-center" text="Get App" />
                            </div>
                        </div>
                ...
                <AnimatePresence>
                    {hoveredItem && (
                        ...
                        >
                            <div className="bg-white/80 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/30 p-3 w-[600px] overflow-hidden">
                                {Object.entries(sectionPreviews).map(([key, section]) => (
                                    hoveredItem === key && (
                                        <div key={key} className="flex flex-col">
                                            {/* Header */}
                                            <div className="px-6 py-5 border-b border-slate-200/50 bg-white/40 backdrop-blur-sm rounded-t-2xl">
                                                <h3 className="font-sora font-bold text-xl text-slate-900">{section.title}</h3>
                                            ...
                                            <div className="grid grid-cols-2 gap-3 p-3">
                                                {section.items.map((item, idx) => (
                                                    <motion.div
                                                        ...
                                                        className="group relative overflow-hidden rounded-2xl bg-white/60 backdrop-blur-sm hover:bg-white border border-slate-200/50 hover:border-slate-300 transition-all duration-300 p-4 flex items-center gap-4 cursor-pointer hover:shadow-xl hover:scale-[1.02]"
                                                    >
                                                        {/* Icon Box */}
                                                        <div className={`w-12 h-12 rounded-xl bg-linear-to-br ${item.gradient} flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                                                            {item.icon}
                                                        </div>
                                                        ...
                                                        {/* Arrow */}
                                                        <ArrowRight className="w-4 h-4 text-slate-300 group-hover:text-teal-600 -translate-x-2 group-hover:translate-x-0 opacity-0 group-hover:opacity-100 transition-all duration-300" />
                                                    </motion.div>
                                                ))}
                                            </div>
                                        </div>
